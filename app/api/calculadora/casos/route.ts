...
import { calculadoraCasoSchema } from "@/lib/validations/calculadora";

// GET /api/calculadora/casos — cotizaciones del usuario logueado (opcional ?deal_id=).
export async function GET(req: NextRequest) {
  const session = await requireAuth(req);
  if (!session) return NextResponse.json({ error: "No autorizado" }, { status: 401 });

  const dealId = req.nextUrl.searchParams.get("deal_id");
  const casos = await prisma.calculadoraCaso.findMany({
...
// POST /api/calculadora/casos — guardar (upsert por nombre: mismo nombre sobrescribe).
export async function POST(req: NextRequest) {
  const session = await requireAuth(req);
  if (!session) return NextResponse.json({ error: "No autorizado" }, { status: 401 });

  const body = await req.json().catch(() => null);
  const parsed = calculadoraCasoSchema.safeParse(body);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const caso = await prisma.calculadoraCaso.upsert({
