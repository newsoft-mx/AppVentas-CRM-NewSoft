                  ...
                  return (
                    <article key={orden.id} className="p-4">
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-3 text-xs text-gray-500">
                        <div>
                          <p className="font-medium uppercase tracking-wide text-gray-500">Tipo</p>
                          <p className="mt-0.5 text-gray-700">{orden.tipo_cotizacion.nombre}</p>
                        </div>
                        <div>
                          <p className="font-medium uppercase tracking-wide text-gray-500">Condición</p>
                          <p className="mt-0.5 text-gray-700">{orden.condicion_pago.nombre}</p>
                        </div>
                        <div>
                          <p className="font-medium uppercase tracking-wide text-gray-500">Fecha</p>
                          <p className="mt-0.5 text-gray-700">{formatFecha(orden.fecha_venta ?? orden.created_at)}</p>
                        </div>
                        <div>
                          <p className="font-medium uppercase tracking-wide text-gray-500">Total</p>
                      ...
                      <div className="mt-4 flex items-center justify-end gap-1.5">
                        <Link href={`/ventas/${orden.id}`} className="rounded-lg p-2 text-gray-500 transition-colors hover:bg-navy/5 hover:text-navy" title="Ver detalle">
                          <Eye size={16} />
                        </Link>
                        <a href={`/api/pdf/${orden.id}`} target="_blank" rel="noopener noreferrer" className="rounded-lg p-2 text-gray-500 transition-colors hover:bg-green-50 hover:text-green-600" title="Descargar PDF">
                          <FileDown size={16} />
                        </a>
                        {canWrite && (
                          <>
                        ...
                        <tr key={orden.id} className="transition-colors hover:bg-gray-50/50">
                          ...
                          <td className="px-5 py-3">
                            <div className="flex items-center justify-end gap-1.5">
                              <Link href={`/ventas/${orden.id}`} className="rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-navy/5 hover:text-navy" title="Ver detalle">
                                <Eye size={14} />
                              </Link>
                              <a href={`/api/pdf/${orden.id}`} target="_blank" rel="noopener noreferrer" className="rounded-lg p-1.5 text-gray-500 transition-colors hover:bg-green-50 hover:text-green-600" title="Descargar PDF">
                                <FileDown size={14} />
                              </a>
                              {canWrite && (
                                <>
